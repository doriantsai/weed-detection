...
precision = dict()  # NOTE: ask David, why dictionary? what more freedom does it grant over list?
recall = dict()
average_precision = dict()
for i in range(len(CLASSES)):
    precision[i], recall[i], _ = precision_recall_curve(bin_classes[:, i], predictions[:, i])
    average_precision[i] = average_precision_score(bin_classes[:, i], predictions[:, i])

precision['micro'], recall['micro'], _ = precision_recall_curve(bin_classes.ravel(), predictions.ravel())
average_precision['micro'] = average_precision_score(bin_classes, predictions, average="micro")
...
colors = cycle(['pink', 'blue', 'green', 'yellow', 'cyan', 'red', 'purple', 'orange', 'grey'])
...
for i, color in zip(range(len(CLASSES)), colors):
    ...
    lines.append(l)
    labels.append('pr for class {0} (ap={1:0.2f}), {2}'.format(i, average_precision[i], CLASS_NAMES[i]))
...
save_img_path = os.path.join('output', data_file[:-4] + '_prcurves.png')
